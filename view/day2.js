...
class Weather extends Component{
  ...
  render() {
    // 遍历 weather 获取返回的一个view（slides）
    const slides = this.state.weather.map((elem, index) => {

      // 遍历 hours 获取返回的一个view的数组 (hours)
      const hourView = elem.hours.map((hourElem, hourIndex) => {
        return (
          //今天各个时刻的天气情况的View
          //[styles.withinDayHoursBox,{backgroundColor: 'red'}
      ...
      return (
        <View style = {{flex: 1}} key={elem.key}>
          <Image style={[styles.image, {flex: 1}]} source={elem.bg}></Image>
...
//</View>
      ...
      <View style={{flex: 1}}> 
        ...
        showsButtons={false}
        paginationStyle={{bottom:10, paddingTop:10, borderTopColor:"red",borderTopWidth:Util.pixel}}
